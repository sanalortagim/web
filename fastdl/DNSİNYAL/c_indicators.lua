...
-- Coded for Valhalla MTA Roleplay server.
-- (Because Fenix can't code).
--
-- Licensed under the BSD license conditions.
--
-- Uses client-side synced element data to sync the
-- indicators state. The element data names can be
-- either 'i:left' and 'i:right', being possible to
-- have both switched on. A true value on an element
-- data means that the indicator is activated. False
-- or nil means deactivated.
--
-- * Version 1.1:
--   - Added tick sounds
--   - Added auto switching off when taking a turn
--

--[[ Configuration ]]--
local INDICATOR_SIZE = 0.32						-- Size for the corona markers
local INDICATOR_COLOR = { 255, 100, 10, 255 }	-- Color in R G B A format
local INDICATOR_FADE_MS = 150					-- Miliseconds to fade out the indicators
...
addEventHandler('onClientResourceStart', getResourceRootElement(getThisResource()), function ()
	...
	bindKey ( "8", "down", handleKeyBind )
	bindKey ( "9", "down", handleKeyBind )
end, false)
