...
local tollinfo = guiCreateLabel(0.34, 0.58, 0.42, 0.04,"Tek Sefer Geçiş Ücreti: $",true,tollGUI)
local tollinfoPrice = guiCreateLabel(0.53, 0.58, 0.42, 0.04,"",true,tollGUI)
local tollPay = guiCreateButton(0.17, 0.66, 0.64, 0.14,"Birkez Geçeceğim",true,tollGUI)
local tollImpulses = guiCreateButton(0.17, 0.82, 0.64, 0.14,"HGS Kart Almak İstiyorum.",true,tollGUI)
	...
	function (state, tollPrice, guest)
		...
		if state == 1 then
			if not guiGetVisible (tollGUI) then
				...
				guiSetText (impulsesAdd, "Kart Başına($"..tollPrice..")")
		...
		if button == "left" and state == "up" then
			...
			elseif source == impulsesAdd then
				if getPlayerMoney (getVehicleController (getPedOccupiedVehicle (localPlayer))) < tonumber (guiGetText (tollinfoPrice)) then
					outputChatBox ("[HGS] Üzgünüm Yeterli Paran Yok Pis Fakir Seni !", 255, 0, 0)
					guiSetVisible (impulsesGUI, false)
					showCursor (false)
				else
